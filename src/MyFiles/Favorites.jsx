...
export default function Favourites() {
    ...
    const handleFileAction = async (action) => {
        ...
        switch (action) {
            case "download": {
                const response = await customFetch(`http://${key}/get_file?file_id=${openFile.file_id || openFile.id}`, {
                    method: "GET",
            ...
            case "delete": {
                const response = await customFetch(`http://${key}/delete_file?${openFile.file_id || openFile.id}`, {
                    method: "DELETE",
                    headers: { 'Content-Type': 'application/json' },
                })
                if (response.ok) {
            ...
            case "favorites": {
                ...
                try {
                    const response = await customFetch(`http://${key}/change_data/file/${targetId}`, {
                        method: "PATCH",
                        headers: { 'Content-Type': 'application/json' },
        ...
        <div className="page">
            ...
            <div id="currentPathDisplay">
                {currentPath.length === 0 && !openFile ? (
                    ...
                    <Link to="/favorites" onClick={() => { setSearchQuery(""); setOpenFile(null); }}>Favourites</Link>
                )}
