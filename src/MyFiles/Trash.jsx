...
import TrashBin from "../assets/SVG/FileIcons/Trash.svg?react"
...
const calculateFolderSize = (folderPath, allFiles) => {
    ...
    return allFiles
        .filter(file => file.inTrash == 1 && file.path.startsWith(folderPath + "/"))
...
export default function Trash() {
    ...
    const visibleFolders = (folders || []).filter(folder => {
        if (folder.inTrash == 0 || getParentPath(folder.path) !== currentPathStr) return false
    ...
    const visibleFiles = (files || []).filter(file => {
        if (file.inTrash == 0 || getParentPath(file.path) !== currentPathStr) return false
