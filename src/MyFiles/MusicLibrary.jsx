...
const calculateFolderSize = (folderPath, allFiles) => {
    ...
    return allFiles
        .filter(file => (viewerComponentsMap[file.extension] == "AudioPlayer") 
...
export default function MusicLibrary() {
    ...
    const { sizeByTypes, totalFileSize } = useMemo(() => {
        ...
        files.forEach(file => {
            if (viewerComponentsMap[file.extension] != "AudioPlayer") return
    ...
    const handleFileAction = async (action) => {
        ...
        switch (action) {
            ...
            case "delete": {
                const response = await customFetch(`http://${process.env.HOST_ADDRESS}:18080/delete_file?${openFile.file_id || openFile.id}`, {
                    method: "DELETE",
                    headers: { 'Content-Type': 'application/json' },
                })
                if (response.ok) {
            ...
            case "favorites": {
                ...
                try {
                    const response = await customFetch(`http://${process.env.HOST_ADDRESS}:18080/change_data/file/${targetId}`, {
                        method: "PATCH",
                        headers: { 'Content-Type': 'application/json' },
