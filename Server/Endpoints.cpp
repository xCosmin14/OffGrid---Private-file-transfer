...
Async<HttpResponse> ClientController::handleRequest(http::verb method, std::string_view target, std::string body, std::string session_id)
{
	...
	{
		if (target == "/register") co_return co_await this->registerUser(obj); // tested

		else if (target == "/log_in") co_return co_await this->loginUser(obj); // tested

		else if (target == "/log_out") co_return co_await this->logoutUser(obj, session_id); // tested

		else if (target == "/delete_account") co_return co_await this->removeUser(obj, session_id); // tested
	...
	else if (method == http::verb::get)
	{
		...
		if (target == "/get_profile_photo") co_return co_await this->getProfilePhoto(session_id); // tested
...
Async<HttpResponse> ClientController::handleRequest(http::verb method, std::string_view target, std::vector<uint8_t>& body, std::string session_id)
{
	...
	if (method == http::verb::post)
	{
		if (target == "/upload_photo") co_return co_await this->uploadFile(body, session_id, "/profile_photos"); // tested

		else if (target.starts_with("/upload_file")) // tested
		{
